...
void X86MCInstLower::Lower(const MachineInstr *MI, MCInst &OutMI) const {
  ...
  //young
  if(MI->getPatch()) {
      AsmPrinter.Patchs.push_back(std::pair<const MCSymbol*, const MCSymbol*>(AsmPrinter.CurrentFnSym, MI->getSymbol()));
  }
  if(MI->getConFlow()){
      AsmPrinter.Patchs.push_back(std::pair<const MCSymbol*, const MCSymbol*>(AsmPrinter.CurrentFnSym, MI->getSymbol()));
      if(MI->isCall()){
          const MCSymbol* sym = &dyn_cast<MCSymbolRefExpr>(OutMI.getOperand(0).getExpr())->getSymbol();
          AsmPrinter.Patchs.push_back(std::pair<const MCSymbol*, const MCSymbol*>(sym,sym));
      }else{
          const MCSymbol* sym = &dyn_cast<MCSymbolRefExpr>(OutMI.getOperand(0).getExpr())->getSymbol();
          AsmPrinter.Patchs.push_back(std::pair<const MCSymbol*, const MCSymbol*>(AsmPrinter.CurrentFnSym, sym));
      }
  }
#if 0
   if(MI->getPatch()) {
       /*
   if(AsmPrinter.MF->getFunction()->hasComdat())
   {
    const MCExpr *DotExpr = MCSymbolRefExpr::create(MI->getSymbol(), AsmPrinter.OutContext);
    const MCExpr *PICBase =
      MCSymbolRefExpr::create(AsmPrinter.CurrentFnSym, AsmPrinter.OutContext);
    DotExpr = MCBinaryExpr::createSub(DotExpr, PICBase, AsmPrinter.OutContext);

    DotExpr = MCBinaryExpr::createAdd(PICBase,
                                      DotExpr, AsmPrinter.OutContext);


      AsmPrinter.Patchs.push_back(DotExpr);
   }else*/
       AsmPrinter.Patchs.push_back(MCSymbolRefExpr::create(MI->getSymbol(), AsmPrinter.OutContext));
  ...
  if(MI->getConFlow()) {
      if(!OutMI.getOperand(0).isExpr())
      {
        const TargetInstrInfo *TII = MI->getParent()->getParent()->getSubtarget().getInstrInfo();

          dbgs() << "ERROR type is not expr " << MI->getOperand(0).getType() <<" " <<MI->getOpcode() << " " <<X86::TAILJMPr64<<TII->getName(MI->getOpcode())<< '\n';

      }

    
      /*if(AsmPrinter.MF->getFunction()->hasComdat() &&!MI->isCall()){
    const MCExpr *DotExpr = OutMI.getOperand(0).getExpr();
    const MCExpr *PICBase =
      MCSymbolRefExpr::create(AsmPrinter.CurrentFnSym, AsmPrinter.OutContext);
    DotExpr = MCBinaryExpr::createSub(DotExpr, PICBase, AsmPrinter.OutContext);
    ...
    DotExpr = MCBinaryExpr::createAdd(PICBase,
      ...
      AsmPrinter.Patchs.push_back(OutMI.getOperand(0).getExpr());
  }
#endif
...
void X86AsmPrinter::EmitInstruction(const MachineInstr *MI) {
  ...
  //young
 // if(MI->getLabel())
 //   OutStreamer->EmitLabel(MI->getSymbol());
