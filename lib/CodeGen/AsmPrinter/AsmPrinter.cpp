...
#ifdef NX_CHECK
#ifdef TARGET_CEHCK
       EmitInt32(0x00a0908a);
       EmitInt32(0xfa800000);
       EmitInt8(0xe);
       EmitInt16(0x0e75);
       EmitInt32(0xf0408b48);
       EmitInt32(0x75c63948);
       EmitInt32(0xfdc9e805);
       EmitInt32(0x5e5affff);
       EmitInt16(0x9d58);
#else
   //if(!(MF->getFunction()->hasComdat()))
       //EmitInt16(0xcccc);
       EmitInt32(0xc3c3c3c3);
       EmitInt32(0xc3c3c3c3);
       EmitInt32(0xc3c3c3c3);
       EmitInt32(0xc3c3c3c3);
#endif
...
void AsmPrinter::EmitFunctionBody() {
  ...
  for (auto &MBB : *MF) {
    ...
    EmitBasicBlockStart(MBB, FirstMBB);
    for (auto &MI : MBB) {
...
#if 0
     if(MI.getPatch()) {
   if(0&&MF->getFunction()->hasComdat())
   {
    const MCExpr *DotExpr = MCSymbolRefExpr::create(MI.getSymbol(), OutContext);
    ...
    DotExpr = MCBinaryExpr::createAdd(PICBase,
      ...
      Patchs.push_back(DotExpr);
   }else
       Patchs.push_back(MCSymbolRefExpr::create(MI.getSymbol(), OutContext));
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  //  for (auto MBB = MyMF->begin(); MBB != MyMF->end(); MBB++) 
   //     for (auto MI = MBB->begin(); MI != MBB->end(); MI++) 
   //         if(MI->isCall())

  // young
  OutStreamer->SwitchSection(Ctx.getObjectFileInfo()->getPatchSection());
  for(auto sym : Patchs)
  {
      
     //OutStreamer->EmitValue(MCSymbolRefExpr::create(sym.first,Ctx), sizeof(void *));
    //OutStreamer->EmitValue(MCSymbolRefExpr::create(sym,Ctx), sizeof(void *));
    //dbgs() << "##@ " << sym.first->getName().str() << " " <<sym.second->getName().str() <<'\n';
     OutStreamer->EmitValue(MCSymbolRefExpr::create(sym.first,Ctx), sizeof(void *));
    if(sym.first == sym.second)
     OutStreamer->EmitIntValue(0, sizeof(void *));
    else{
    const MCExpr *DotExpr = MCSymbolRefExpr::create(sym.second, OutContext);
    const MCExpr *PICBase =
      MCSymbolRefExpr::create(sym.first, OutContext);
    DotExpr = MCBinaryExpr::createSub(DotExpr, PICBase, OutContext);
    ...
    OutStreamer->EmitValue(DotExpr, sizeof(void *));
    }
  }
...
/// it if appropriate.
void AsmPrinter::EmitBasicBlockStart(const MachineBasicBlock &MBB, bool FirstMBB) const {
